...
const SupervisorDashboard = () => {
    ...
    const [selectedCategories, setSelectedCategories] = useState([]); // RENAMED
    const [showCategoryHelp, setShowCategoryHelp] = useState(false); // RENAMED

    // NEW DYNAMIC TAXONOMY STATE
    const [categoryOptions, setCategoryOptions] = useState([]); // RENAMED
        ...
        const fetchTaxonomies = async () => {
            try {
                ...
                if (response.data.status === 'success') {
                    setCategoryOptions(response.data.categories.map(item => ({ value: item, label: item }))); // RENAMED
        ...
        try {
            try {
                ...
                if (profileRes.data.status === 'success') {
                    ...
                    setSelectedCategories(p.project_category ? p.project_category.map(f => ({value: f, label: f})) : []); // RENAMED
                }
            } catch (err) {
    ...
    const handleSaveProfile = async (e) => {
        ...
        const payload = {
            ...
            primary_project_category: selectedCategories.map(f => f.value), // RENAMED
        };
